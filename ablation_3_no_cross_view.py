...
class Token_Embedding(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        # Move inputs to the same device as Longformer to avoid device mismatch errors
...
class Dual_View_Model(nn.Module):
    ...
    def forward(self, batch):
        # 1. Use the model's device so any tensors we create are on the same
        # device as the model (Longformer moves its inputs internally).
