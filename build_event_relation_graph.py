...
class Event_Identification(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, label_event):
        ...
        return event_weighted_loss, event_raw_scores, event_embeddings


class Event_FactInterp(nn.Module):
    def __init__(self):
        super(Event_FactInterp, self).__init__()
    ...
    def forward(self, event_embeddings):
        fact_interp_scores = self.fact_interp_head_2(self.relu(self.fact_interp_head_1(event_embeddings)))
        return fact_interp_scores
...
event_fact_interp = Event_FactInterp()
event_fact_interp.cuda()
try:
    if os.path.exists('./saved_models/event_relation_graph/best_macro_F_fact_interp.ckpt'):
        event_fact_interp.load_state_dict(torch.load('./saved_models/event_relation_graph/best_macro_F_fact_interp.ckpt', map_location=device))
        print("Loaded factual/interpretive classifier from best_macro_F_fact_interp.ckpt")
    elif os.path.exists('./saved_models/event_relation_graph/best_macro_F_graph.ckpt'):
        full_state = torch.load('./saved_models/event_relation_graph/best_macro_F_graph.ckpt', map_location=device)
        fact_interp_state = {k.replace('fact_interp_head_', ''): v for k, v in full_state.items() if 'fact_interp' in k}
        event_fact_interp.load_state_dict(fact_interp_state, strict=False)
        print("Loaded factual/interpretive classifier from best_macro_F_graph.ckpt")
    else:
        print("WARNING: No factual/interpretive model found - will skip classification")
        event_fact_interp = None
except Exception as e:
    print(f"WARNING: Could not load factual/interpretive model: {e}")
    event_fact_interp = None

if event_fact_interp is not None:
    event_fact_interp.eval()
    ...
    def forward(self, input_ids, attention_mask, label_event, event_pairs, label_coreference):
        ...
        for event_pair_i in range(event_pairs.shape[0]):
            if event_pair_i == 0:
                ...
                event_pair_embeddings = torch.cat((event_1_embedding, event_2_embedding, pair_element_wise_sub, pair_element_wise_mul), dim = 1)
            else:
                event_1_in_label_event = event_pairs[event_pair_i, 0]
                event_2_in_label_event = event_pairs[event_pair_i, 1]
                event_1_embedding = event_embeddings[event_1_in_label_event, :].view(1, 768)
        ...
        coreference_raw_scores = self.coreference_head_3(self.relu(self.coreference_head_2(self.relu(self.coreference_head_1(event_pair_embeddings)))))
        coreference_loss = self.crossentropyloss(coreference_raw_scores, label_coreference)
        
        coreference_loss_weight_0 = (label_coreference == 0).int()
        coreference_loss_0 = torch.mul(coreference_loss, coreference_loss_weight_0)
        
        coreference_loss_weight_1 = torch.mul(label_coreference, coreference_weight_positive)
...
class Event_Temporal(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, label_event, event_pairs, label_temporal):
        ...
        temporal_raw_scores = self.temporal_head_3(self.relu(self.temporal_head_2(self.relu(self.temporal_head_1(event_pair_embeddings)))))
        temporal_loss = self.crossentropyloss(temporal_raw_scores, label_temporal)
        
        temporal_loss_weight_0 = (label_temporal == 0).int()
...
class Event_Causal(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, label_event, event_pairs, label_causal):
        ...
        causal_raw_scores = self.causal_head_3(self.relu(self.causal_head_2(self.relu(self.causal_head_1(event_pair_embeddings)))))
        causal_loss = self.crossentropyloss(causal_raw_scores, label_causal)
        
        causal_loss_weight_0 = (label_causal == 0).int()
...
class Event_Subevent(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, label_event, event_pairs, label_subevent):
        ...
        subevent_raw_scores = self.subevent_head_3(self.relu(self.subevent_head_2(self.relu(self.subevent_head_1(event_pair_embeddings)))))
        subevent_loss = self.crossentropyloss(subevent_raw_scores, label_subevent)
        
        subevent_loss_weight_0 = (label_subevent == 0).int()
...
# PART 2: BASIL Dataset Processing
...
for file_idx in range(len(file_names)):
    ...
    index_of_token = 0
    sent_dict = {'sentence_id': 'title', 'sentence_text': original_article_json['title'] + '.', 'tokens': [], 'label_info_bias': -1, 'label_info_lex_bias': -1}
    ...
    for sent_i in range(len(article_json['sentences'])):
        ...
        for token_i in range(len(article_json['sentences'][sent_i]['tokens'])):
            ...
            if start + len(word_encoding['input_ids']) < MAX_LEN:
    ...
    label_event = torch.tensor(label_event)
    ...
    with torch.no_grad():
        event_weighted_loss, event_raw_scores, event_embeddings = event_identification(input_ids, attention_mask, label_event)
        ...
        for token_i in range(len(article_json['sentences'][sent_i]['tokens'])):
            ...
            if index_of_token < event_predicted_prob.shape[0]:
                ...
                if article_json['sentences'][sent_i]['tokens'][token_i]['prob_event'][0] < 0.5:
                    ...
                    event_embeddings_list.append(event_embeddings[index_of_token])
            else:
                stop_flag = 1
                break
    ...
    if event_fact_interp is not None and len(event_embeddings_list) > 0:
        event_embs_tensor = torch.stack(event_embeddings_list, dim=0)
        
        with torch.no_grad():
            fact_interp_scores = event_fact_interp(event_embs_tensor)
        
        fact_interp_probs = softmax(fact_interp_scores)
        fact_interp_labels = torch.argmax(fact_interp_probs, dim=1)
        
        fact_interp_probs = fact_interp_probs.to('cpu').numpy()
        fact_interp_labels = fact_interp_labels.to('cpu').numpy()
        
        for idx, event_token in enumerate(event_tokens):
            event_token['prob_fact_interp'] = [float(fact_interp_probs[idx][0]), float(fact_interp_probs[idx][1])]
            event_token['label_fact_interp'] = int(fact_interp_labels[idx])
            event_token['fact_interp_class'] = 'factual' if fact_interp_labels[idx] == 0 else 'interpretive'
...
# PART 3: BiasedSents Dataset Processing
...
for file_idx in range(len(file_names)):
    ...
    article_json['sentences'] = []
    index_of_token = 0
    sent_dict = {'sentence_id': 'ref_title', 'sentence_text': original_article_json['ref_title'] + '.', 'tokens': [], 'label_bias': -1}
    tokens_list = nltk.tokenize.TreebankWordTokenizer().tokenize(sent_dict['sentence_text'])
    for token_i in range(len(tokens_list)):
        token_dict = {'index_of_token': index_of_token, 'token_text': tokens_list[token_i], 'prob_event': [0, 0], 'label_event': 0}
        sent_dict['tokens'].append(token_dict)
    ...
    sent_dict = {'sentence_id': 0, 'sentence_text': original_article_json['title'] + '.', 'tokens': [], 'label_bias': original_article_json['ann_title']}
    tokens_list = nltk.tokenize.TreebankWordTokenizer().tokenize(sent_dict['sentence_text'])
    for token_i in range(len(tokens_list)):
        token_dict = {'index_of_token': index_of_token, 'token_text': tokens_list[token_i], 'prob_event': [0, 0], 'label_event': 0}
        sent_dict['tokens'].append(token_dict)
