#!/usr/bin/env python3
"""
Baseline 2: Feature Concatenation (Simple Graph)
This model removes all sophisticated GNNs and simply concatenates
the Longformer+BiLSTM sentence embedding with an aggregated
vector of all events in that sentence.
"""
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description='Baseline 2: Feature Concatenation for Media Bias Detection',
    ...
    parser.add_argument('--results_dir', type=str, default='./results_baseline_2',
...
print("="*70)
print("BASELINE 2: FEATURE CONCATENATION (SIMPLE GRAPH)")
print("="*70)
if torch.cuda.is_available():
...
class custom_dataset(Dataset):
    """
    This dataset loader remains unchanged from the full script.
    It loads all event and relation data, which is needed
    for the event_agg calculation.
    """
    ...
    def __getitem__(self, idx):
        ...
        # --- Relation data is loaded but will be ignored by this baseline model ---
        event_pairs_raw = []
...
class Dual_View_Model(nn.Module):
    """
    BASELINE 2: Feature Concatenation Model
    All GNNs are removed.
    Final classifier input is [sentence_embedding, event_aggregation_embedding]
    """
    def __init__(self):
        ...
        # Classifier
        # Input dimension is feature_dim * 2 (sentence_emb + event_agg)
    ...
    def forward(self, batch):
        ...
        # --- All GNN Processing Removed ---
        
        # --- Event Aggregation (as planned for Baseline 2) ---
        event_agg = torch.zeros_like(sentence_embeddings).to(current_device)
        if event_words.size(0) > 0:
            # Get event embeddings (simple mean of tokens)
            event_embeddings_list = []
            for i in range(event_words.size(0)):
                start, end = int(event_words[i, 0]), int(event_words[i, 1])
                event_embeddings_list.append(torch.mean(token_embeddings[start:end], dim=0))
            event_embeddings = torch.stack(event_embeddings_list)
            
            # Aggregate events by sentence
            min_sent = int(event_words[:, 2].min())
            max_sent = int(event_words[:, 2].max())
            for sent_idx in range(min_sent, max_sent + 1):
                if sent_idx >= event_agg.size(0): # Ensure sent_idx is in bounds
                    break
        ...
        # Concatenate sentence embedding with event aggregation
...
if __name__ == "__main__":
    ...
    summary = {
        ...
        'method': 'Baseline 2: Feature Concatenation (Simple Graph)',
        'hyperparameters': {
            'epochs': args.epochs,
            'batch_size': args.batch_size,
            'max_len': args.max_len,
    ...
    output_filename = f"{RESULTS_DIR}/final_results_baseline_2.json"
    with open(output_filename, 'w') as f:
        json.dump(summary, f, indent=2)

    print("\n" + "="*70)
    print(f"FINAL RESULT ({args.n_folds} FOLDS) - BASELINE 2")
