...
"""
    ...
    def process_member_files(self, member_dir: str):
        ...
        # Calculate optimal batch size to fit under 1000 requests
        optimal_batch_size = max(10, (config['estimated_events'] + 999) // 1000)
        estimated_requests = (config['estimated_events'] + optimal_batch_size - 1) // optimal_batch_size
        ...
        if estimated_requests > 1000:
            print(f"WARNING: Still exceeds quota! Need {estimated_requests} requests")
            print(f"Consider increasing batch size further or splitting data more")
            return False
        ...
        for filename in tqdm(filenames, desc=f"Member {self.member_id}"):
            ...
            try:
                ...
                if events:
                    # Use optimal batch size to stay under quota
                    labels = self._classify_article_events(events, batch_size=optimal_batch_size)
    ...
    def _classify_article_events(self, events: List[Dict], batch_size=10) -> List[int]:
        """Classify events in batches to stay under 1,000 RPD quota"""
        all_labels = []
        ...
        for i in range(0, len(events), batch_size):
            ...
            if self.daily_requests >= self.max_daily_requests:
                ...
                print("Stopping to stay within free tier limits")
                break
            ...
            try:
                ...
                print(f"API error for member {self.member_id}: {e}")
